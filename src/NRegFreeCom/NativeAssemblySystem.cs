...
{
    public class NativeAssemblySystem
    {
        public string Win32Directory = "Win32";
        public string x64Directory = "x64";

        public NativeAssembly LoadAnyCpuSubLibrary(string name)
        {
            return LoadAnyCpuLibrary(AppDomain.CurrentDomain.BaseDirectory, name);
        }

        public NativeAssembly LoadAnyCpuLibrary(string directoryPath, string name)
        {
            ...
            return new NativeAssembly(hModule, name, path);
        }

        public NativeAssembly LoadLibrary(string path)
        {
            ...
            if (hModule == IntPtr.Zero) throw new Win32Exception(Marshal.GetLastWin32Error());
            return new NativeAssembly(hModule, Path.GetFileName(path), path);
        }
    }
}
